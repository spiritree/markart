...
export default class articleRelease extends PureComponent {
  ...
  async componentDidMount() {
    ...
    const { descript, email, sub_title, title, url } = this.props.option.data.option.result;
  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll(['title', 'sub_title', 'url', 'descript', 'email'], (error) => {
        const body = this.props.form.getFieldsValue(['title', 'sub_title', 'url', 'descript', 'email']);
        if (!error) {
          dispatch({
    ...
    const updateValidate = () => {
      validateFieldsAndScroll(['username', 'oldPassword', 'newPassword'], (error) => {
        const body = this.props.form.getFieldsValue(['username', 'oldPassword', 'newPassword']);
              ...
              <Form hideRequiredMark layout="vertical">
                ...
                <Form.Item {...formItemLayout} label="站点描述">
                  {getFieldDecorator('descript', {
                    ...
                    <TextArea placeholder="站点描述" autosize={{ minRows: 2 }} />
                  )}
                </Form.Item>
            ...
            <Card title="个人信息" className={styles.card} bordered={false}>
              <Form hideRequiredMark>
                ...
                <Form.Item {...formItemLayout} style={{ float: 'right' }}>
                  <Button type="primary" onClick={updateValidate} loading={updating}>修改</Button>
                </Form.Item>
              </Form>
            </Card>
          </Col>
